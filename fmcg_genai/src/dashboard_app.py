...
class EnhancedFMCGDashboard:
    ...
    def create_kpi_cards(self, cleaned_df):
        ...
        with col3:
            price_range = cleaned_df['price_unit'].max() - cleaned_df['price_unit'].min()
            st.metric(
                label="💵 Average Price",
                value=f"${avg_price:.2f}",
                delta=f"Range: ${price_range:.2f}"
            )
    ...
    def create_insights_section(self, cleaned_df):
        ...
        with col2:
            ...
            price_volume_corr = cleaned_df[['price_unit', 'units_sold']].corr().iloc[0, 1]
            avg_price = cleaned_df['price_unit'].mean()
            high_price_sales = cleaned_df[cleaned_df['price_unit'] > avg_price]['units_sold'].mean()
            low_price_sales = cleaned_df[cleaned_df['price_unit'] <= avg_price]['units_sold'].mean()
            
            st.write(f"**Correlation:** {price_volume_corr:.2f} {'(Strong)' if abs(price_volume_corr) > 0.5 else '(Moderate)'}")
            st.write(f"**Above Avg Price:** {high_price_sales:.0f} units/day")
            st.write(f"**Below Avg Price:** {low_price_sales:.0f} units/day")
    ...
    def create_advanced_visualizations(self, cleaned_df):
        ...
        # Create subplot with secondary y-axis (removed promotions)
        fig = make_subplots(
            rows=1, cols=1,
            subplot_titles=('Daily Sales Volume & Price Trends',),
        ...
        # Moving average (changed from MA30 to full name)
        daily_sales['Moving_Average_30'] = daily_sales['units_sold'].rolling(window=30).mean()
        fig.add_trace(
            go.Scatter(x=daily_sales['date'], y=daily_sales['Moving_Average_30'],
                      name='30-Day Moving Average', line=dict(color='#ff7f0e', width=2, dash='dash')),
            secondary_y=False
        )
        ...
        # Better metrics instead of regional performance
        col1, col2 = st.columns(2)
        
        with col1:
            st.markdown("### 📊 Top Products by Revenue")
            # Calculate revenue per product
            product_revenue = cleaned_df.groupby('sku').agg({
                'units_sold': 'sum',
                'price_unit': 'mean'
            }).reset_index()
            product_revenue['revenue'] = product_revenue['units_sold'] * product_revenue['price_unit']
            product_revenue = product_revenue.nlargest(10, 'revenue')
            
            fig = px.bar(product_revenue, x='sku', y='revenue',
                        color='units_sold', color_continuous_scale='Blues',
                        title='Top 10 Products by Revenue',
                        labels={'revenue': 'Revenue ($)', 'units_sold': 'Units Sold', 'sku': 'Product'})
        ...
        with col2:
            ...
            monthly_sales = cleaned_df.groupby(cleaned_df['date'].dt.to_period('M'))['units_sold'].sum()
            ...
            fig.add_trace(go.Bar(
                ...
                marker_color=['green' if x > 0 else 'red' for x in monthly_growth.values],
                name='Growth %'
            ))
            fig.update_layout(
    ...
    def create_qa_portal(self):
        ...
        with col2:
            ...
            if st.button("Top selling products?", key="q4"):
                st.session_state.user_query = "Which products had the highest sales volume?"
